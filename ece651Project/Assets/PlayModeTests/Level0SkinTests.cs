...
public class Level0SkinTests
{
    ...
    public IEnumerator SkinChangeTest()
    {
        ...
        Assert.AreEqual("1_Idle", anim.GetCurrentAnimatorClipInfo(0)[0].clip.name);

        //test level 2 skin
        script.ChangeCharSkin(2);
        yield return null;
        Assert.AreEqual("Level2Override", anim.runtimeAnimatorController.name);
        Assert.AreEqual("2_Idle", anim.GetCurrentAnimatorClipInfo(0)[0].clip.name);

        //test level 3 skin
        script.ChangeCharSkin(3);
        yield return null;
        Assert.AreEqual("Level3Override", anim.runtimeAnimatorController.name);
        Assert.AreEqual("3_Idle", anim.GetCurrentAnimatorClipInfo(0)[0].clip.name);

        //test level 4 skin
        script.ChangeCharSkin(4);
        yield return null;
        Assert.AreEqual("Level4Override", anim.runtimeAnimatorController.name);
        Assert.AreEqual("4_Idle", anim.GetCurrentAnimatorClipInfo(0)[0].clip.name);
